...
Predictive pursuit estimates target velocity and turn rate, projects curved motion, and selects the earliest speed-feasible intercept -- but only trusts that forecast while the search actually converges within a horizon that is itself scaled down when the Runner's turn rate has been flip-flopping (reacting/juking) rather than genuinely arcing. When the forecast is not currently trustworthy, the Catcher falls back to `CHASE` (aim directly at the Runner's current position) instead of steering at an arbitrary extrapolation, with the switch itself debounced so it cannot chatter every tick. When the Runner shields behind a lidar-mapped obstacle, the Catcher enters `FLANK` -- only once the Runner has genuinely lingered near it, not on a single-tick pass-through -- commits to one orbit direction, and cuts around the obstacle rather than settling opposite the Runner; a single commitment is capped in duration and cools down if it runs long, so it cannot be baited into an indefinite orbit. Both roles pass their tactical goal through an adaptive lidar navigator: it evaluates 48 possible body-width corridors, balances goal progress against clearance, retains its chosen side, and reverses/reorients after detecting a stall. Modes expose this as `/GAP` or `/RECOVERY`. During final approach the Catcher distinguishes the Runner surface from a wall so safety does not block legal capture. RGB-D color/depth tracks the red or blue role marker and can restore the opponent pose if the primary pose stream becomes stale; a plausibility check rejects any detection implying a jump the robot could not physically make, and the color thresholds require a low off-hue channel so a saturated orange/magenta surface (e.g. a painted obstacle) cannot be mistaken for the role marker. The Runner's own evasion (`strategic`/`adversarial`/`competitive`) holds a randomly-weighted choice among its top-scoring escape headings for a short window rather than always the single best one, so a deterministic policy cannot be fully predicted by an opponent simulating the same scoring function; a safety-critical `BREAKAWAY` always takes the single best escape.
...
The committed equal-speed benchmark was regenerated with 10 seeds, 180-second limits, eight Runner behaviors, and both Catcher strategies: 160/160 cases captured. Mean capture time was 11.32 s for baseline and 12.75 s for predictive (previously 13.72 s, before the turn-consistency-gated horizon, feasibility self-arbitration, and dwell-gated flanking described above -- `predictive` still trails `baseline` on strategic/adversarial evasion, just by less). These kinematic results validate termination and regression coverage; they do not rank the strategies under physical obstacles and sensor latency.
...
Implemented: one-world/two-namespace launch, autonomous independent commands, replaceable state adapter, three Catcher strategies and six Runner strategies, curvature-aware interception with turn-consistency-gated horizon and feasibility self-arbitration, dwell-gated anti-shield flanking, mixed-strategy evasion, deterministic A*, live lidar safety, RGB-D fallback tracking with a detection plausibility gate, sensor watchdogs, continuous capture evaluation, genuinely-measured collision/path metrics, JSON/CSV results, seeded benchmark gates, markers, YAML tuning, three sensor profiles, and automated algorithm/ROS/profile tests.

The primary simulator pose adapter still uses Gazebo ground truth because the official technical interface has not been released; RGB-D is a tested fallback, not yet the sole localization source. A* is tested but is not fed the live occupancy map; local RPLidar navigation and obstacle mapping are active. Predicted path/escape markers are not all separately rendered. Official sensor restrictions and submission interfaces remain rulebook-dependent. The latest two-robot `full`-profile headless physics gate used random seed 909 and equal 0.70 m/s ceilings: capture at 15.099 s, 0.352 m minimum separation, and zero collisions -- **this predates the turn-consistency/self-arbitration/flank-dwell/mixed-strategy changes above**, which are validated on the kinematic benchmark only so far; re-run this Gazebo gate before relying on the 15.099 s figure again.

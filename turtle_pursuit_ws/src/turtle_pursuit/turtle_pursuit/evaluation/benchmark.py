...
def run(catcher_strategy,behavior,seed,duration=60.):
    random.seed(seed); c=Pose2D(-1.5,0,0,0); r=Pose2D(1.5,0,math.pi,0); cs=CatcherStrategy(C_CFG); rs=RunnerStrategy(R_CFG,seed); est=VelocityEstimator(.35); det=CaptureDetector(.5,1.); dt=.05; t=0.; cp=rp=0.; minimum=99.
    while t<duration:

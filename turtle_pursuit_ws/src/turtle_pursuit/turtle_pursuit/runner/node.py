...
class RunnerNode(Node):
    def __init__(self):
        super().__init__('runner_controller'); self.declare_parameter('strategy','strategic'); self.declare_parameter('seed',1); self.declare_parameter('control_rate',20.0); self.declare_parameter('require_lidar',False); self.declare_parameter('require_camera',False); self.declare_parameter('sensor_timeout',1.0); self.declare_parameter('sensor_startup_grace',5.0)
        vals={'max_linear':.70,'cruise_linear':.44,'runner_boost_distance':3.0,'max_angular':1.8,'linear_accel':1.2,'angular_accel':2.8,'stale_timeout':.6,'arena_half':5.0,'boundary_margin':.55,'lookahead':1.25,'turn_gain':2.2,'distance_weight':1.0,'clearance_weight':1.7,'open_weight':.6,'smooth_weight':.35,'emergency_escape_distance':1.15,'adversarial_break_weight':.9,'adversarial_interval':.8,'survival_radial_weight':3.5,'safe_feint_distance':2.6,'shield_radius':1.05,'shield_reach_weight':1.6,'shield_lost_margin':.35,'shield_switch_hysteresis':.75,'shield_association_distance':.75,'shield_join_ratio':1.45,'shield_join_step':.65,'shield_orbit_step':.48,'obstacle_map_resolution':.15,'obstacle_map_range':4.5,'obstacle_wall_margin':.70,'obstacle_min_cluster_cells':4,'obstacle_map_min_range':.12,'obstacle_map_ttl':15.0,'obstacle_opponent_exclusion':.48,'obstacle_max_component_span':2.2,'obstacle_bridge_gap_cells':2,'lidar_stop_distance':.650,'lidar_influence_distance':1.350}
        vals.update({'navigator_robot_radius':.32,'navigator_clearance_cap':3.0,'navigator_stall_window':1.25,'navigator_stall_distance':.055,'navigator_recovery_time':.9,'navigator_heading_samples':48,'navigator_clearance_weight':1.35,'navigator_goal_weight':2.1,'navigator_continuity_weight':.75})
    ...
    def tick(self):
        ...
        if sensor_fault: cmd=Command(); mode='SENSOR_FAULT'
            ...
            if scan and self.strategy.target is not None:
                ...
                cmd,navigation=self.navigator.command(cmd,runner,*scan[:3],target_bearing=bearing,now=now)

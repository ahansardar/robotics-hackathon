...
class CatcherNode(Node):
    def __init__(self):
        super().__init__('catcher_controller'); self.declare_parameter('strategy','predictive'); self.declare_parameter('control_rate',20.0); self.declare_parameter('require_lidar',False); self.declare_parameter('require_camera',False); self.declare_parameter('sensor_timeout',1.0); self.declare_parameter('sensor_startup_grace',5.0)
        ...
        vals.update({'boundary_margin':.55,'navigator_robot_radius':.32,'navigator_clearance_cap':3.0,'navigator_stall_window':1.25,'navigator_stall_distance':.055,'navigator_recovery_time':.9,'navigator_heading_samples':48,'navigator_clearance_weight':1.35,'navigator_goal_weight':2.1,'navigator_continuity_weight':.75,'catcher_navigator_robot_radius':.29,'catcher_navigator_clearance_weight':1.05,'catcher_navigator_goal_weight':3.2,'catcher_navigator_continuity_weight':.5})
